...
const ROUTE_PARAMETER_MATCHER = "ＲＯＵＴＥ－ＰＡＲＡＭＥＴＥＲ－ＭＡＴＣＨＥＲ";
...
module.exports = async ({
  ...
  let isHttp = noct.request.isHttp = event.httpMethod ? true : false;
  let data = {};
  ...
  if (isHttp) {
    ...
    route = noct.request.route = event.path === '/' ? '/' : event.path.substring(1, event.path.length);
    method = noct.request.httpMethod = event.httpMethod.toUpperCase();
    ...
    noct.routes.forEach(x => {
      ...
      if (serviceRoute && serviceRoute != '/') {
        if (serviceRoute.startsWith("/") || serviceRoute.endsWith('/')) {
          ...
          throw new Error(
            `路由格式错误，每个参数匹配符之间需要包含其它字符，以便对路由进行区分。${JSON.stringify({
							service: serviceName,
							route: serviceRoute
						})}`
          );
        ...
        x.routes.forEach(xx => {
          ...
          if (xx.httpMethod) {
            ...
            if (actionName.toUpperCase().startsWith("GET")) {
              ...
              actionName.toUpperCase().startsWith("UPDATE")) {
              httpMethod = ["PUT"];
    ...
    routeTemplates = routeTemplates.sort((a, b) => {
      ...
      if (arr.length < brr.length) {
        ...
        var arsl = a.routeTemplate.replace(new RegExp("\\{[^\\}]+\\}", "g"), ROUTE_PARAMETER_MATCHER).split(ROUTE_PARAMETER_MATCHER).length - 1;
        var brsl = b.routeTemplate.replace(new RegExp("\\{[^\\}]+\\}", "g"), ROUTE_PARAMETER_MATCHER).split(ROUTE_PARAMETER_MATCHER).length - 1;
        if (arsl > brsl) {
          return true;
        }
      }
    ...
    var repeatRouteTemplates = routeTemplates.filter(x => {
      if (matchedRouteTemplate.routeTemplate.replace(
          new RegExp("\\{[^\\}]+\\}", "g"),
          ROUTE_PARAMETER_MATCHER) === x.routeTemplate.replace(new RegExp("\\{[^\\}]+\\}", "g"),
          ROUTE_PARAMETER_MATCHER
        )) {
        return x;
      }
    });
    ...
    var _b = _a.split('/');
    var _c = _a.replace(new RegExp("\\{[^\\}]+\\}", "g"), ROUTE_PARAMETER_MATCHER).split('/');
    var _d = route.split('/');
    _c.forEach((x, xi) => {
      var y = _d[xi];
      var z = _b[xi];
      if (x.includes(ROUTE_PARAMETER_MATCHER)) {
        if (x === ROUTE_PARAMETER_MATCHER) {
          // {id} 5
          routeParams[z.replace(new RegExp("\\{|\\}", "g"), '')] = y;
        } else {
          // student:{name}-{age} student:Sansnn-99
          var _f = x.split(ROUTE_PARAMETER_MATCHER);
          var _g = y;
          var _r = z;
    ...
    noct.request.routeTemplate = matchedRouteTemplate.routeTemplate;
  }
  // 不是URL化则使用匹配模式
  else {
    noct.mode = "match";

    service = event[noct.config.serviceKey];
    action = event[noct.config.actionKey];
    data = event[noct.config.dataKey] || {};

    if (noct.config.enableMatchMode) {
      if (!service) {
        if (noct.mode === "match") {
      ...
      // 主动忽略
      var matchIgnore = noct.config.matchIgnore.filter(x => x.service === service);
  ...
  // 参数整理
  event[noct.config.serviceKey] = noct.request.service = service;
  event[noct.config.actionKey] = noct.request.action = action;
  event[noct.config.dataKey] = noct.request.data = data;

  // 实例化service
  let serviceObj = require(`${noct.config.baseDir}${noct.config.serviceDir}${service}`);
  ...
  // 执行action方法
  try {
    var response = await serviceInstance[action](noct.request.data);
    if (response) {
      // 响应结果
      noct.response.body = response;
    }
  } catch (exception) {
    // 异常处理
